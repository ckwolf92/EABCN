...
liq_constraint = 1; % is the liquid wealth constraint active?

xkink1M     = 0.10*4; % ratio = 0 up to kink1 (multiple mean gross labinc)
xkink2M     = 10*4;   % ratio = liqratiomax over kink2, linear in between
liqratiomax = 0.5; 

xkink1 = xkink1M * mean_y;
xkink2 = xkink2M * mean_y;

if liq_constraint == 1

    liqratiofn = @(x)  (liqratiomax.*(x-xkink1)./(xkink2-xkink1)).*(x>xkink1).*(x<xkink2)+ liqratiomax.*(x>=xkink2);
    liqratioderivfn = @(x) (liqratiomax./(xkink2-xkink1)).*(x>xkink1).*(x<xkink2);
...
liqgrid      = liqratiofn(grid_a').*grid_a';
liqderivgrid = liqratiofn(grid_a') + liqratioderivfn(grid_a').*grid_a';

grid_b            = liqgrid';
grid_db           = liqderivgrid';
grid_b_rep        = repmat(liqgrid,n_y,1);
grid_db_rep       = repmat(liqderivgrid,n_y,1);
...
[c_opt,ap_opt,mutilde_upd] = EGPliq_fun(mutilde_opt,r_hat_SS,0,grid_b_rep,grid_db_rep,...
                                        1,1,0,beta_hat,gamma,0,states,grid_a,n_yT,Emat_yP);
...
[c_opt_MPC,~,~] = EGPliq_fun(mutilde_opt,r_hat_SS,0,grid_b_rep,grid_db_rep,...
    1,1,gift,beta_hat,gamma,0,states,grid_a,n_yT,Emat_yP);
